...
function findmatch(array) {
    ...
    for(i = 0; i < array.length; i++) {
        if(comparearray.length > 2) {
            var phrase = (datafull.slice( (comparearray[0].index-1), (comparearray[comparearray.length-2].index+1)) ).join(" ");
            console.log(phrase);
        ...
        if( index === 0 ) { index = tempindex } else {
            if( tempindex < index ) {
                ...
                matchingString = (datafull.slice( (comparearray[0].index-1), ([comparearray.length-3].index-1))).join(" ");
        ...
        for(j=counter; j < arr2.length; j++) {
            ...
            if (temparray.indexOf(currentword) == -1 ) {
                ...
                tempindex = [comparearray.length-1].index - comparearray[0].index;
            }
        }
    counter++;
     }
